...
> **Aggiornamento 31 ago 2026:** 8 voci chiuse nella sessione «discipline di realismo» — verificate
> nel **realism lab** (`?lab=1`, chunk lazy: l'entry non lo vede), su WebGPU reale, 0 errori
> console, screenshot con pixel veri. Restano aperte: transmission/SSS, grounding shadows,
> gesture example, scroll/pointer hooks riusabili, CI browser-smoke, rubrica auditor estesa.
...
- [x] *(31 ago 26 — `src/canvas/InstancedField.tsx`, nel lab con 4000 istanze)* **Ship `InstancedField.tsx`**: native `THREE.InstancedMesh` scatter + per-instance attributes (hash/wind-phase/color), TSL node material, tier-gated count, paired with a drei `<Detailed>` hero. The "instancing exists in code" proof. `[geometry] (M · high)`
- [x] *(31 ago 26 — `tsl/triplanar.ts` + `materials/pbrTriplanar.ts`; triplanar delle NORMALI escluso di proposito, vedi header)* **PBR + triplanar material primitive** `materials/pbrTriplanar.ts` (KTX2 maps → colorNode/normalNode/roughnessNode; `triplanar(map,scale,sharpness)` blending `abs(normalWorld)`). The #1 anti-tiling win. `[materials] (M · high)`
- [x] *(31 ago 26 — `materials/alphaModes.ts`: hash/cutout/coverage con i requisiti di AA in tabella)* **Alpha-edge recipe** `materials/alphaModes.ts` (alphaHash: `transparent=false`,`depthWrite=true`, needs TAA — confirmed vs the official example; alphaToCoverage needs MSAA; alphaTest for cutouts). `[materials] (S · high)`
...
- [x] *(31 ago 26 — `PostFX.tsx`: MRT output+normalView → GTAO RedFormat-safe → bloom(full) → SMAA, tier-gated, render a priority 1; DoF e grade/LUT restano da aggiungere)* **Reusable PostFX module** `PostFX.tsx`: `RenderPipeline` + `pass` + MRT(`output`,`normalView`) → GTAO (`ao` node, RedFormat-safe `color.mul(vec3(ao.r))`) → bloom → SMAA → tonemap → grade. Tier-gated (full/medium/off). `[post] (L · high)`
- [x] *(31 ago 26 — `tsl/wind.ts` a 3 strati con maschera d'altezza; consumer = InstancedField)* **Multi-layer TSL wind node** `tsl/wind.ts` (global sway masked by height + gust fronts + per-instance turbulence) + a `FoliageField.tsx` consumer. Makes the foliage doc's "concrete next step" runnable. `[animation/materials] (M · high)`
...
- [x] *(31 ago 26 — rapier in deps + `physics/PhysicsStage.tsx` steppato follow; semantica reduced scelta: la fisica NON monta. ecctrl rimandato al primo brief con personaggio)* **Add `@react-three/rapier` (+ ecctrl)** to deps — confirmed **absent today** — and `PhysicsStage.tsx`: `<Physics>` stepped inside the R3F frame, 1–2 `<RigidBody>` heroes, optional character. Rapier is the **sole** owner of body transforms (never also damped). `[animation] (M · high)`
...
- [x] *(31 ago 26 — `AdaptiveQuality.tsx`: PerformanceMonitor→setDpr nel range del tier, un solo owner; AdaptiveDpr scartato di proposito, sarebbe un secondo scrittore)* **Adaptive layer**: wrap scene in drei `<PerformanceMonitor onDecline>` + `<AdaptiveDpr pixelated>` + `<AdaptiveEvents>`. All confirmed in drei 10.7. `[perf] (M · high)`
- [x] *(31 ago 26 — `hooks/useRenderBackend.ts` su backend.isWebGPUBackend)* **WebGPU/WebGL2 feature-gate seam** `useRenderBackend()` reading `renderer.isWebGPURenderer`. *Note:* the flag stays `true` on WebGL2 fallback — for true backend detection read `renderer.backend.isWebGPUBackend`. `[perf] (S · medium)`
